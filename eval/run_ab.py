...
def _write_ab_report(report: Dict[str, Any], reports_dir: Path) -> Path:
    ...
    lines = [
        ...
        f"## Aggregate Delta",
        f"",
        f"| Metric | Baseline (A) | Rerank (B) | Δ (B − A) |",
        f"|--------|-------------|-----------|-----------|",
        ...
        f"",
        f"> Enable `RAG_RERANK_ENABLED=true` only when Δ nDCG@{k} > 0 at acceptable latency cost.",
        f"",
...
def main() -> int:
    ...
    print(f"{'Metric':<22} {'Baseline (A)':>14} {'Rerank (B)':>12} {'Δ (B−A)':>10}")
    print("-" * 62)
    for label, ak, bk in [
